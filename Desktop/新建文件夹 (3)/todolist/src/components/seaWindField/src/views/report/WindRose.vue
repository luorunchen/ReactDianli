...
export default {
  ...
  methods: {
    ...
    getPointData () {
      ...
      this.$axios
        ...
        .then(response => {
          // 隐藏loading框
          this.$loading1().close()
          if (response.data.length !== 73) {
            return
          }
          // 处理风频图数据
          this.initBarData(response.data)

          // 风玫瑰图数据处理
          this.initRadarDta(response.data)
        })
        .catch((error) => {
          // 隐藏loading框
          this.$loading1().close()
    ...
    /**
     * 根据选择的风机，整合查询的参数
     * @return {String} 返回处理好的查询参数
     */
    initSearchParams () {
      // eslint-disable-next-line prefer-const
      let params = []
      const searchTime = this.$moment(new Date(this.startTime).getTime() + 1000 * 60 * 60).format('YYYYMMDDHHmmss') + '-' + this.$moment(this.endTime).format('YYYYMMDDHHmmss')
      // 初始化风向相关测点
      this.FXStr.forEach(s => {
        this.FX.forEach(x => {
          let p
          if (s !== '') {
            p = 'ZQ:F' + this.currentFan.id + 'WSPEED_' + x + s
          } else {
            p = 'ZQ:F' + this.currentFan.id + 'WDIR2_' + x + s
          }
          params.push(searchTime + ',' + p + ',' + 'ZQF' + this.currentFan.id + ',1')
        })
      })
      // 初始化风速测点
      for (let i = 0; i < 25; i++) {
        const p = i * 5 > 5 ? i * 5 : (i * 5 + '').padStart(2, '0')
        params.push(searchTime + ',' + 'ZQ:F' + this.currentFan.id + 'WSPEED_COUNT' + p + ',' + 'ZQF' + this.currentFan.id + ',1')
      }
      return params.join(';')
    },
    /**
     * 初始化风频图数据
     * @param {Array} l 测点返回的数据
     */
    initBarData (l) {
      // 用来初始化统计对象
      // eslint-disable-next-line prefer-const
      ...
      for (let i = 48; i < 73; i++) {
        let num = 0
        const d = l[i][Object.getOwnPropertyNames(l[i])]
        for (const k in d) {
          num += Number(d[k].sum)
        }
        obj[(i - 48 * 0.5)] = num
      ...
      // eslint-disable-next-line prefer-const
      let percentage = { 类别: '次数' }
      // eslint-disable-next-line prefer-const
      let windSpeed = { 类别: '风速' }

      this.tableCol2.forEach(e => {
        const field = e * 10 + ''
        percentage[field] = obj[e]
        windSpeed[field] = e
        this.bar_series.push(obj[e])
      })
    ...
    initRadarDta (l) {
      ...
      let counts = 0
      // 初始化时间变量，用来计算平均风速
      const st = this.$moment(this.startTime).format('YYYYMMDD000000')
      const et = this.$moment(this.endTime).format('YYYYMMDD000000')
      const sh = 24 - new Date(this.startTime).getHours()
      const eh = new Date(this.endTime).getHours()
      // 统计各个风向出现的次数，以及对应的风速数据
      this.FX.forEach((x, i) => {
        obj[x] = {}
        let max = -Infinity
        // 取最大风速
        let d = l[i][Object.getOwnPropertyNames(l[i])]
        for (const k in d) {
          max = max > d[k].max ? max : d[k].max
        }
        obj[x].max = max
        // 取平均风速
        let h = 0
        let sum = 0
        d = l[i + 16][Object.getOwnPropertyNames(l[i + 16])]
        for (const k in d) {
          // 根据开始时间和结束时间的具体小时值，计算权重
          if (st === k) {
            h += sh
            sum = sh * Number(d[k].avg)
          } else if (et === k) {
            h += eh
            sum = eh * Number(d[k].avg)
          } else {
            h += 24
            sum = 24 * Number(d[k].avg)
          }
        }
        obj[x].avg = sum / h
        // 总次数
        sum = 0
        d = l[i + 32][Object.getOwnPropertyNames(l[i + 32])]
        for (const k in d) {
          sum += Number(d[k].sum)
        }
        counts += sum
      ...
      this.FX.forEach(n => {
        ...
        this.tableData1[2][n] = (obj[n].counts / counts * 100).toFixed(2)
        // 找出雷达图中的最大值
    ...
    checkSearch () {
      ...
      if (Date.now() <= new Date(this.endTime).getTime()) {
        if (this.dataTypeValue === '0') {
          this.endTime = this.$moment(new Date()).format('YYYY/MM/DD HH:00:00')
    ...
    dataTypeValueChange (val) {
      ...
      if (val === '0') {
        ...
        this.valueDataFormat = 'yyyy/MM/dd HH:00:00'
